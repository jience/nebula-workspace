      ...
      <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl p-6 animate-in zoom-in-95 duration-200">
        <div
          className={`w-12 h-12 rounded-full flex items-center justify-center mb-4 ${
            isForce ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-500' : 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-500'
          }`}
        >
          <AlertTriangle size={24} />
        </div>
        <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-2">{isForce ? 'Force Power Off?' : 'Shutdown Virtual Machine?'}</h3>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 px-4 py-2 text-white rounded-lg text-sm font-medium transition-colors shadow-lg ${
              isForce ? 'bg-red-600 hover:bg-red-500 shadow-red-600/20' : 'bg-amber-600 hover:bg-amber-500 shadow-amber-600/20'
            }`}
          >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="p-3 bg-slate-50 dark:bg-slate-800/30 rounded-lg border border-slate-200 dark:border-slate-800">
              ...
              <div className="text-lg font-semibold text-slate-900 dark:text-white truncate" title={resource.os}>
                {resource.os}
              </div>
            </div>
            <div className="p-3 bg-slate-50 dark:bg-slate-800/30 rounded-lg border border-slate-200 dark:border-slate-800">
              ...
              <div className="text-lg font-semibold text-slate-900 dark:text-white truncate" title={resource.region}>
                {resource.region.split(' ')[0]}
              </div>
            </div>
          </div>
...
const Dashboard: React.FC<DashboardProps> = ({ onLaunch, category, searchQuery }) => {
  ...
  const filterResources = (resList: VDIResource[]) => {
    ...
    return resList.filter((res) => res.name.toLowerCase().includes(lowerQuery) || res.os.toLowerCase().includes(lowerQuery) || res.region.toLowerCase().includes(lowerQuery))
  }
  ...
  return (
    <div className="relative space-y-8 animate-fade-in pb-20">
      ...
      <div
        className={`fixed top-14 right-0 bottom-8 w-80 bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl border-l border-slate-200 dark:border-slate-700 shadow-2xl z-30 transform transition-transform duration-300 ease-in-out ${
