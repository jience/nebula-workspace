import { AlertCircle, AlertTriangle, Check, Keyboard, Loader2, Maximize2, Minimize2, MousePointer2, Power, Settings, Signal, Wifi, X } from 'lucide-react'
import React, { useEffect, useState } from 'react'
import pic3 from '../assets/res-5.jpg'
import { ConnectionQuality, SessionStatus, VDIResource } from '../types'
...
const VDIViewer: React.FC<VDIViewerProps> = ({ resource, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black flex flex-col font-sans">
      ...
      <div
        className={`h-10 bg-slate-900 border-b border-slate-700 flex items-center justify-between px-4 transition-transform duration-300 z-[60] relative ${
          isFullscreen && !showInputSettings ? '-translate-y-full hover:translate-y-0' : ''
        }`}
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          <button onClick={toggleFullscreen} className="p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 rounded" title="Toggle Fullscreen">
            {isFullscreen ? <Minimize2 size={16} /> : <Maximize2 size={16} />}
          </button>
          <button onClick={handleDisconnect} className="p-1.5 text-red-400 hover:text-red-300 hover:bg-red-900/30 rounded flex items-center gap-1 text-xs font-medium">
            <Power size={14} />
            Disconnect
          </button>
        </div>
      ...
      <div className="flex-1 relative flex items-center justify-center bg-slate-800 overflow-hidden">
        ...
        {status === SessionStatus.CONNECTED && (
          <div className="w-full h-full bg-cover bg-center relative group cursor-none" style={{ backgroundImage: `url('${pic3}')` }}>
            ...
            <div className="absolute top-20 left-20 w-1/2 h-1/2 bg-slate-800 rounded-lg shadow-2xl border border-slate-600 flex flex-col overflow-hidden">
              ...
              <div className="flex-1 bg-white p-8 text-slate-900">
                ...
                <p className="text-sm text-gray-600">
                  This is a simulated view of a remote desktop session. In a production environment, this canvas would render a WebRTC stream or an HTML5 Canvas drawing raw pixel data from the
                  RDP/PCoIP protocol.
                </p>
              </div>
            </div>
